...
router.post('/apply_loan', function(req, res, next) {
	con.connect(function(err){
		if(err) throw err;
		var sql = "INSERT INTO accepted (repayment_schedule,lender_name,lender_eth_address,borrower_eth_address,amount,duration,your_story) VALUES ('yearly','','','"+req.body.borrower_eth_address+"','"+req.body.amount+"','"+req.body.duration+"','"+req.body.your_story+"')";
		con.query(sql, function (err2, result){
			if(err2) throw err2;
			res.redirect('/thanks');
		});
